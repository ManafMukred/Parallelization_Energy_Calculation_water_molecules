...
main(){
  int i,j,natoms,nmol,npairs=0,nthreads,me;
  ...
  double read_wall_time,read_cpu_time,calc_wall_time,calc_cpu_time,Ttime;
...
////////////////////begin second timer ///////////////////////
  ...
  int calc_npairs,npair_shared[12],chunk=100;
  calc_npairs =  nmol*  (nmol-1)/2 ;

  printf("calculated npairs %i \n",calc_npairs);
#pragma omp parallel private(nthreads,me,npairs)
{
me = omp_get_thread_num();   /* Obtain thread number */
  ...
  #pragma omp for  schedule(dynamic,chunk) reduction(+:energy) 
...
npair_shared[me]=npairs;
printf("thread %i npairs  = %i\n",me,npairs);
...
///////////// load imbalance //////////////////////////////////////////////////////
int actual_npairs=0,maxnpair=npair_shared[0],minnpair=npair_shared[0];
double imbalance;
for(i=0; i<12; i++)
    {
        // printf("ele = %i \n",npair_shared[i]);

        actual_npairs = actual_npairs + npair_shared[i];        
        /* If current element is greater than max */
        if(npair_shared[i] > maxnpair)
        {
            maxnpair = npair_shared[i];
        }

        /* If current element is smaller than min */
        if(npair_shared[i] < minnpair)
        {
            minnpair = npair_shared[i];
        }

    }
imbalance = 2.*(maxnpair-minnpair)/(0.+maxnpair+minnpair);
printf("actual_npairs = %i \n",actual_npairs);
printf("max = %i \n",maxnpair);
printf("min = %i \n",minnpair);
printf("imbalance = %lf \n",imbalance);
